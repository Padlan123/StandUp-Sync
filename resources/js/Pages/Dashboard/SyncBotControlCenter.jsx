...
export default function SyncBotControlCenter({ auth, groups, botConfig, recentLogs }) {
    const [isCollapsed, setIsCollapsed] = useState(false);
    const [botEnabled, setBotEnabled] = useState(botConfig?.enabled ?? true);
    const [reminderTime, setReminderTime] = useState(botConfig?.reminderTime ?? '09:00');
                    ...
                    <div className="max-w-4xl mx-auto">
                        ...
                        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-[0_2px_8px_-2px_rgba(0,0,0,0.05)] mb-6">
                            <div className="flex items-center justify-between">
                                <div>
                                    <h2 className="text-base font-semibold text-slate-900">SyncBot Automation</h2>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-[0_2px_8px_-2px_rgba(0,0,0,0.05)]">
                                ...
                                <div className="space-y-3">
                                    {[
                                        { label: 'In-App Chat', sub: 'Send reminder inside Briefly chat', checked: true },
                                        { label: 'Discord Bot', sub: 'Forward reminder to Discord channel', checked: true },
                                    ].map((item, i) => (
                                        <label key={i} className="flex items-center justify-between cursor-pointer group">
                                            ...
                                            <div className={`w-9 h-5 rounded-full flex items-center px-0.5 transition-colors ${item.checked ? 'bg-emerald-500' : 'bg-slate-200'}`}>
                                                <div className={`w-4 h-4 bg-white rounded-full shadow transition-transform ${item.checked ? 'translate-x-4' : 'translate-x-0'}`}></div>
                                            </div>
                                        </label>
                                    ))}
                                </div>
